...
class LetterMorseViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        currentString = chooseRandomString(mode: characters)
    ...
    @IBAction func endCharPressed(_ sender: UIButton) {
        if isMorseSequenceValid(input: morseInput.text!) {
            if morseToChar(morseString: morseInput.text!) == currentChar {
                ...
                if self.currentIndex == self.currentString.endIndex {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 1.0){
                        ...
                        // Select new entry
                        self.currentString = self.chooseRandomString(mode: self.characters)
                    ...
                    // Increment indexes for next access
                    self.currentChar = String(self.currentString[self.currentIndex])
                }
